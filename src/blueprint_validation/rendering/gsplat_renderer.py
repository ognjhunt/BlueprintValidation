...
def render_frame(
    ...
    render_mode = "RGB+ED"
    # gsplat appends depth channels internally for RGB+D / RGB+ED, so we pass RGB only.
    expected_background_channels = 3
    ...
    if background is None:
        ...
        if background.shape[0] != expected_background_channels:
            raise ValueError(
