    ...
    with episodes_jsonl.open("w") as f:
        for entry in rollouts:
            ...
            payload = {
                ...
                "target_instance_id": str(entry.get("target_instance_id", "") or ""),
                "target_label": str(entry.get("target_label", "") or ""),
                "initial_camera": dict(entry.get("initial_camera", {}) or {}),
                "path_context": dict(entry.get("path_context", {}) or {}),
                "source_type": str(entry.get("source_type", "policy_rollout") or "policy_rollout"),
                "sim_backend": str(entry.get("sim_backend", "") or ""),
                "success_source": str(entry.get("success_source", "") or ""),
                "parent_rollout_id": str(entry.get("parent_rollout_id", "") or ""),
                "motion_family": str(entry.get("motion_family", "") or ""),
                "task_score": float(entry.get("task_score", 0.0)),
                "success": success,
                "task_success": bool(entry.get("task_success", False)),
                "task_success_available": bool(entry.get("task_success_available", False)),
                "task_success_reason": str(entry.get("task_success_reason", "") or ""),
                "steps": steps,
                "manipulation": {
