"""HTTP client for the downstream site-world runtime service.

The wire protocol is shared across native and legacy runtime implementations.
This module is the validation harness entrypoint; implementation-specific
...
class RuntimeServiceClientConfig:
    service_url: str
    api_key: str
    timeout_seconds: int

    @classmethod
    def from_env(cls) -> "RuntimeServiceClientConfig":
        timeout_raw = (
            _env("SITE_WORLD_RUNTIME_SERVICE_TIMEOUT_SECONDS")
            or _env("NEOVERSE_RUNTIME_SERVICE_TIMEOUT_SECONDS")
            or "120"
        )
        return cls(
            service_url=(
                _env("SITE_WORLD_RUNTIME_SERVICE_URL")
                or _env("NEOVERSE_RUNTIME_SERVICE_URL")
            ).rstrip("/"),
            api_key=(
                _env("SITE_WORLD_RUNTIME_SERVICE_API_KEY")
                or _env("NEOVERSE_RUNTIME_SERVICE_API_KEY")
            ),
...
class RuntimeServiceClient:
    def __init__(self, config: RuntimeServiceClientConfig) -> None:
        self.config = config
    ...
    def render_bytes(self, session_id: str, *, camera_id: str) -> bytes:
        return self._request_bytes(
            method="GET",
            path=f"/v1/sessions/{urllib_parse.quote(session_id)}/render/{urllib_parse.quote(camera_id)}",
            accept="image/png",
        )
    ...
    def explorer_frame_bytes(self, session_id: str, *, camera_id: str) -> bytes:
        return self._request_bytes(
            method="GET",
            path=f"/v1/sessions/{urllib_parse.quote(session_id)}/explorer/frame/{urllib_parse.quote(camera_id)}",
            accept="image/png",
        )
    ...
    def stream_session(
        ...
        *,
        ...
        ws_base = self.runtime_info().get("websocket_base_url") or self.config.service_url.replace("http://", "ws://").replace("https://", "wss://")
        url = f"{str(ws_base).rstrip('/')}/v1/sessions/{urllib_parse.quote(session_id)}/stream"
        count = 0
        with websocket_connect(url, open_timeout=self.config.timeout_seconds) as websocket:
            while True:
                message = websocket.recv(timeout=self.config.timeout_seconds)
