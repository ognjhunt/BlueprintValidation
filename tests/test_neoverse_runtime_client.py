...
def test_runtime_client_covers_json_and_bytes(monkeypatch):
    ...
    def _urlopen(request, timeout):
        ...
        if url.endswith("/v1/site-worlds/siteworld-1/sessions"):
            return _FakeResponse(json.dumps({"session_id": "session-1"}).encode("utf-8"))
        if url.endswith("/v1/sessions/session-1/reset"):
            return _FakeResponse(json.dumps({"episode": {"stepIndex": 0, "done": False}}).encode("utf-8"))
        if url.endswith("/v1/sessions/session-1/step"):
            return _FakeResponse(json.dumps({"episode": {"stepIndex": 1, "done": False}}).encode("utf-8"))
        if url.endswith("/v1/sessions/session-1/state"):
            return _FakeResponse(json.dumps({"status": "running", "step_index": 1}).encode("utf-8"))
