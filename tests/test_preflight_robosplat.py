...
def test_preflight_includes_robosplat_checks(sample_config, monkeypatch):
    ...
    monkeypatch.setattr(
        preflight, "check_external_tool", lambda *a, **k: PreflightCheck("tool", True)
    )
    monkeypatch.setattr(
        preflight, "check_facility_ply", lambda *a, **k: PreflightCheck("ply", True)
    )
    monkeypatch.setattr(
        preflight, "check_model_weights", lambda *a, **k: PreflightCheck("weights", True)
    )
    monkeypatch.setattr(preflight, "check_hf_auth", lambda: PreflightCheck("hf", True))
    monkeypatch.setattr(
        preflight, "check_path_exists", lambda *a, **k: PreflightCheck("path", True)
    )
    monkeypatch.setattr(
        preflight, "check_path_exists_under", lambda *a, **k: PreflightCheck("path_under", True)
    )
    monkeypatch.setattr(
        preflight, "check_python_import_from_path", lambda *a, **k: PreflightCheck("imp", True)
    )
    monkeypatch.setattr(preflight, "check_api_key", lambda *a, **k: PreflightCheck("api", True))
    monkeypatch.setattr(
        preflight, "check_api_key_for_scope", lambda *a, **k: PreflightCheck("api_scope", True)
    )
